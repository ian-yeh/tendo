...
You do not need tendo installed globally. Invoke it with `npx -y @ianyeh/tendo <command>`. The examples below write `tendo <command>` for brevity; run each as `npx -y @ianyeh/tendo <command>` and the CLI comes along on demand.

First use fetches Chromium once (via Playwright). If a command reports the browser is missing, run `npx playwright install chromium`.
...
1. `tendo look <url> --session s1 --annotate` captures state. Writes screenshots to disk and prints an element map (`id, role, name, bbox`), console errors, failed requests, and next-step `hints:`.
2. **Read the annotated screenshot** with your own vision. The numbered boxes map to element ids: search box = `3`, checkout = `1`, etc.
3. `tendo act --session s1 --element 1` clicks that exact element. The response is the fused post-action page state (never a bare "Done").
...
1. For anything past a single glance, use `--session <id>` so the browser persists across your turns. One-shot (no `--session`) launches and kills per call, so element ids from one call are not valid in the next.
...
4. On `not_found`, the returned capture is already fresh, so re-read it and re-target; do not re-run `look`.
...
- Diagnostics (`consoleErrors`, `failedRequests`) are deduped, path-stripped, and capped, so they flag real breakage, not media/tracker noise.
